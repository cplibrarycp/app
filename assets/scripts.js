...
async function fetchLibraryData() {
    ...
    if (cached) {
        window.fullLibrary = JSON.parse(cached);
        renderBooks();
    }
    try {
        ...
        window.fullLibrary = rows.map(cols => ({
            lang: cols[0],
            catKey: (cols[1] || '').toString().trim().toLowerCase(),
            catTitle: cols[2],
            id: cols[3],
            type: (cols[4] || 'pdf').toString().toLowerCase(),
            title: cols[5],
            img: cols[6] || 'assets/cover/default.jpg'
        }));
...
function renderBooks(searchTerm = "") {
    ...
    if (books.length > 0) {
        ...
        container.innerHTML = books.map(book => {
            ...
            return `<div class="book-card"><div class="image-wrapper" style="position:relative; overflow:hidden; border-radius:10px;"><img class="book-cover" src="${book.img}" onerror="this.src='assets/cover/default.jpg'"><div class="read-overlay" id="overlay-${book.id}">${actionBtn}</div><div class="audio-player-container" id="player-${book.id}" style="display:none; position:absolute; bottom:0; left:0; width:100%; height:15%; background:rgba(0, 77, 64, 0.95); z-index:40;"><div style="width:100%; height:100%; position:relative; overflow:hidden;"><iframe id="iframe-${book.id}" src="" style="width:100%; height:350%; border:none; margin-top:-45px;" allow="autoplay"></iframe><div style="position:absolute; top:50%; right:10px; transform:translateY(-50%); background:white; color:#004D40; width:22px; height:22px; border-radius:50%; display:flex; justify-content:center; align-items:center; cursor:pointer; z-index:60; font-size:12px;" onclick="stopMediaInline('${book.id}')"><i class="fas fa-times"></i></div></div></div></div><div class="book-title">${book.title}</div></div>`;
        }).join('');
    }
}
...
function injectMasterTemplate() {
    ...
    const headerHTML = `<header><div class="nav-container">
        <a href="dashboard.html" class="logo-container">
            ...
            <img src="assets/cover/logo.png" class="logo-img">
        </a>
        <div class="nav-links" style="display: flex; align-items: center;">
            <button class="music-btn" onclick="toggleMusic()"><i id="music-icon" class="fas fa-volume-mute"></i></button>
            ...
            <div class="user-profile" id="user-profile-btn" style="display: flex; align-items: center; gap: 5px; cursor: pointer; position: relative;">
                ...
                <span id="display-name" style="font-size:0.85em;">...</span>
                <i class="fas fa-chevron-down" style="font-size:0.6em;"></i>
                ${exitBtn}
                <div class="profile-dropdown" id="profile-dropdown" style="display:none; position:absolute; right:0; top:40px; background:white; box-shadow:0 5px 15px rgba(0,0,0,0.2); border-radius:8px; min-width:130px; overflow:hidden; z-index:2000;">
                    <a href="settings.html" style="display:block; padding:8px 12px; color:#333; text-decoration:none; font-size:0.8em; border-bottom:1px solid #eee;">Settings</a>
                    <a href="history.html" style="display:block; padding:8px 12px; color:#333; text-decoration:none; font-size:0.8em; border-bottom:1px solid #eee;">History</a>
    ...
    const modalsHTML = `<div id="loginAlertModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:10001; justify-content:center; align-items:center;"><div class="popup-box" style="background:white; border-radius:15px; text-align:center; width:220px; border: 1.5px solid #00897B; overflow:hidden; display:flex; flex-direction:column;"><div style="background:#00897B; padding:15px 10px; width:100%;"><img src="assets/cover/logo.png" style="height:35px; margin-bottom:8px;"><h2 style="color:white; margin:0; font-size:0.85em; letter-spacing:1px; text-transform:uppercase; font-weight:700;">THRIPUDI LIBRARY</h2></div><div style="padding:20px 15px;"><p style="color:#333; margin:0 0 15px 0; font-weight:bold; font-size:0.8em;">തുടരുന്നതിന് ലോഗിൻ ആവശ്യമാണ്</p><div style="display:flex; flex-direction:column; gap:8px;"><button onclick="goToLogin()" style="padding:10px; background:#00897B; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:bold;">ലോഗിൻ ചെയ്യുക</button><button onclick="document.getElementById('loginAlertModal').style.display='none'" style="padding:8px; background:white; color:#00897B; border:1px solid #00897B; border-radius:8px; cursor:pointer;">ഇപ്പോൾ വേണ്ട</button></div></div></div></div><div id="mediaModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:#000; z-index:10002; flex-direction:column;"><div style="width:100%; background:#004D40; color:white; padding:15px; display:grid; grid-template-columns: 1fr 2fr 1fr; align-items:center;"><span id="modalBookTitle" style="font-size:0.8em; opacity:0.85; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">...</span><span style="text-align:center; font-weight:bold; font-size:1em;">ത്രിപുടി വായനാമുറി</span><button onclick="closeMediaModal()" style="color:white; background:none; border:none; cursor:pointer; font-size:2em; text-align:right;">&times;</button></div><div style="flex:1; position:relative; overflow:hidden;"><iframe id="mediaFrame" style="width:100%; height:100%; border:none;" allow="autoplay"></iframe><div id="frame-blocker" style="position:absolute; top:0; right:0; width:65px; height:65px; pointer-events:auto; background:transparent; z-index:10003;"></div></div></div>`;

    document.head.insertAdjacentHTML('beforeend', styles);
    document.body.insertAdjacentHTML('afterbegin', headerHTML);
    const mainContent = document.querySelector('.container:not(.navbar):not(.nav-container):not(.footer-container)');
    if (mainContent) {
